...
description: Connect Claude to the Lofty CRM API for real estate agents and VAs. Use whenever the user mentions Lofty, Lofty CRM, or the Lofty API. Tasks include finding a lead, logging a note, scheduling a showing or tour, post-showing SMS, leads index, jotform feedback, error 200058, or webhooks. Trigger on first-time setup ("set up Lofty," "connect my CRM," "I just installed this"), on Cloudflare Worker deploys (Tier 2 post-showing feedback, Tier 3 showing-reminder SMS, Tier 4 live leads index, e.g. "set up Tier 3," "deploy the SMS Worker"), on showing orchestration ("schedule a showing," "book a tour," "set [client] at [address] at [time]"), on workflow questions ("how do I find a lead," "log a note for," "search the MLS"), and on troubleshooting ("Lofty error," "200058," "auth failed"). Trigger even on casual mentions ("the Lofty thing," "my CRM is Lofty," "in Lofty"). Do NOT trigger for general real estate questions unrelated to Lofty.
---
...
- **A showing to schedule end-to-end** ("schedule a showing," "book a tour," "set up [client] at [address] at [time]") → follow the multi-stop orchestration workflow in `references/schedule-showing.md`, which drives it end to end (resolve client, prepare each stop, calendar event, showing-log note, verify SMS queue). Requires `prepare_showing` helpers in the starter (v1.3.0+) and a calendar backend matching `CALENDAR_PROVIDER` in `CLAUDE.md`. The Tier 3 SMS Worker is optional; the orchestration works without it but skips Step 7.
...
- `references/schedule-showing.md` - orchestration workflow that drives multi-stop showing scheduling end to end (resolve client, prepare each stop, create calendar events, post showing-log notes, verify SMS queue). Used whenever the user asks Claude to schedule, book, or set up a showing or tour.
