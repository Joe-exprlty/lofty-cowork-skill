...
38. **`update_lead(tags=[...])` REPLACES the entire tag list.** It is not a merge or an append. The naive "add a tag" pattern `api.update_lead(lead_id, tags=["NewTag"])` silently deletes every other tag the lead had. Confirmed live 2026-05-12. v1.10 adds three safe wrappers (`add_tag`, `remove_tag`, `set_tags`) that read-merge-write so existing tags survive. Always use those wrappers, never `update_lead(tags=...)` directly.

39. **Tag-list items must be tag NAMES (strings), not tagIds (integers).** Passing integer tagIds to `update_lead(tags=[...])` does not look up the names. Instead, Lofty silently CREATES new tags whose names are those integer strings, polluting the team tag library. Confirmed live 2026-05-12: integer payloads produced three orphan tags ("6536952", "903557", "954693") in the team library. The public API does NOT expose tag deletion at any common path (probed: `/v1.0/tags/{id}`, `/v1.0/tag/{id}`, `/v1.0/team-features/tag/{id}`, `/v1.0/agent/{agentId}/tag/{id}` all 404); orphans must be cleaned up via the Lofty web UI (Settings -> Tags). The v1.10 tag-write helpers refuse non-string entries.

40. **Segments are read-only via the public API.** Every lead read returns a `segments` field (Lofty's web UI calls them "groups"), but the public API does NOT expose writes for segments. Segment management routes through the internal API at `crm.lofty.com/api/`, which requires signed requests (per-request `platform`, `timestamp`, and `signature` headers generated by Lofty's web JS) and is not reverse-engineerable in scope. To add or remove a segment on a lead, use the Lofty web UI. The kit surfaces segments read-only via `api.get_lead_segments(lead_id)` and the `segments` field on `find_client` matches.

41. **`/v1.0/leads` also silently ignores `assigneeId`, `stageId`, and `tagId` as query filters.** Confirmed live 2026-05-12. Passing any of these returns the unfiltered first page. This is an expansion of quirk #2; the broader rule is that `/v1.0/leads` accepts query-string filters only for `scrollId` (pagination) and `pageSize` (capped at 25). Every other filter is dropped silently. All lead filtering must be done client-side after pulling the page.
