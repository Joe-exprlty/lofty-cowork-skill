...
Showings are the highest-leverage workflow. The kit ships an orchestration workflow at `references/schedule-showing.md` that wires every step below together. Trigger phrases like "schedule a showing at," "book a tour for," or "set up [client] at [address] at [time]" route directly into it. The notes below are for users who want to compose the primitives themselves, or who are debugging a partial run.
